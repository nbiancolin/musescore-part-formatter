...
def cleanup_mm_rests(staff):
    """
    Go through entire score, remove any "_mm" attributes
    """
    for elem in staff:
...
def add_rehearsal_mark_line_breaks(staff):
    ...
    for i in range(len(list(staff))):
        elem = staff[i]
        if elem.tag != "Measure":
            continue #could be vbox, want to skip that

        assert elem.find("voice") is not None
        if elem.find("voice").find("RehearsalMark") is not None:
            #add rehearsal mark to prev
            if elem.attrib.get("_mm") is not None:
                prev_elem = staff[i - elem.attrib["_mm"]]
            else:
                prev_elem = staff[i -1]
